...
\date{\today}
\titlegraphic{\includegraphics[height=1.7cm]{img/logo_unam.png}\hfill\includegraphics[height=1.7cm]{img/logo_fio.png}}
\begin{document}
...
\begin{frame}{La Selva Misionera}
  ...
  \end{columns}
  \note{
    La Selva Misionera concentra más de la mitad de la biodiversidad del país
    en menos del \qty{0.5}{\percent} del territorio. Destaca por el Corredor Verde que
    conecta áreas protegidas. Es crucial destacar que esta ecorregión
    forma parte de un sistema mucho mayor llamado el Bosque Atlántico.
  }
\end{frame}
...
\begin{frame}{El Bosque Atlántico}
  ...
  \end{columns}
  \note{
    Misiones contiene el remanente en Argentina de este enorme ecosistema llamado Bosque Atlántico,
    uno de los hotspots más amenazados del planeta, del cual queda a lo sumo
    un \qty{17}{\percent} de su extensión original. Por eso su protección es vital.
  }
\end{frame}
...
\begin{frame}{Limitaciones de Camaras Tradicionales}
  ...
  \vspace{0.5em}
  \centering
  \begin{alertblock}{El problema de la latencia}
    ¿Cuánto tiempo tarda entre que se detecta un evento de interés y nos enteramos? \textbf{Mucho tiempo}.
  \end{alertblock}

  \vspace{0.5em}
  \centering
  \textit{Para cuando se detecta una intrusión,\\los responsables ya están lejos del área.}
  \note{
...
\begin{frame}{Live Demo del Sistema}
  ...
  \fbox{\parbox{0.8\textwidth}{\centering\vspace{3cm}
      [Espacio para demostración en vivo]\\
      (Activación de cámara $\rightarrow$ Mesh $\rightarrow$ Telegram)
  \vspace{3cm}}}
  \note{
    En este punto realizamos la demostración en vivo. Activamos una cámara,
    la petición viaja por la red mesh al servidor local, procesa la imagen
    y llega la notificación al bot de Telegram.
  }
\end{frame}
